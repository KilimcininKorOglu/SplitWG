.PHONY: build build-amd64 build-arm64 build-universal \
        ...
        notarize-app notarize-dmg sign-minisign
...
define CODESIGN_APP
	@if [ -n "$(DEV_ID)" ]; then \
	    echo "codesign helper: Developer ID — $(DEV_ID)"; \
	    codesign --sign "$(DEV_ID)" --options runtime \
	             --identifier com.kilimcininkoroglu.splitwg-helper \
	             --force --timestamp \
	             $(1)/Contents/MacOS/splitwg-helper; \
	    echo "codesign app: Developer ID — $(DEV_ID)"; \
	    codesign --sign "$(DEV_ID)" --options runtime \
...
# DMGs must be codesigned separately so Gatekeeper can validate the disk
# image itself (`spctl -t open`). No --options runtime, no --entitlements,
# no --deep — those apply only to executables and break DMG signatures.
# Skip on ad-hoc keychains — an unsigned DMG is fine for local smoke tests.
define CODESIGN_DMG
	@if [ -n "$(DEV_ID)" ]; then \
	    echo "codesign dmg: Developer ID — $(DEV_ID)"; \
	    codesign --sign "$(DEV_ID)" --force --timestamp $(1); \
	else \
	    echo "codesign dmg: skipped (no Developer ID in keychain)"; \
	fi
endef
...
dmg-intel: bundle-intel
	...
	$(call CODESIGN_DMG,$(DMG_INTEL))

# Apple Silicon-only DMG (smaller download for arm64 users).
dmg-arm: bundle-arm
	mkdir -p dist
	bash scripts/make-dmg.sh "$(APP_ARM)" "$(DMG_ARM)" "SplitWG"
	$(call CODESIGN_DMG,$(DMG_ARM))

# ─── Notarization + minisign ──────────────────────────────────────────────────
#
# Two stages. Apple's notarization ticket lives with whichever artifact
# was stapled, so both the .app (for when users copy it out of the DMG)
# and the DMG (for when Gatekeeper checks the download) must be stapled
# independently.
#
# notarize-app: zip each .app, submit to notarytool, wait, then staple the
# ticket into the bundle. Must run BEFORE `dmg*` targets package the .app.
#
# notarize-dmg: submit each DMG (which now contains a pre-stapled .app),
# wait, staple the DMG's own ticket. Apple only needs the DMG codesign +
# embedded .app notarization to pass.
#
# The notary profile must be pre-seeded via
# `xcrun notarytool store-credentials $(NOTARY_PROFILE) …`. See README.
notarize-app: bundle-universal bundle-intel bundle-arm
	@set -euo pipefail; \
	for app in $(APP) $(APP_INTEL) $(APP_ARM); do \
	    if [ -d "$$app" ]; then \
	        echo "notarize: $$app"; \
	        /usr/bin/ditto -c -k --sequesterRsrc --keepParent \
	            "$$app" "/tmp/$$app.zip"; \
	        submission_id=$$(xcrun notarytool submit "/tmp/$$app.zip" \
	            --keychain-profile "$(NOTARY_PROFILE)" --wait \
	            --output-format json | tee /dev/stderr | \
	            /usr/bin/python3 -c 'import sys,json; print(json.load(sys.stdin)["id"])'); \
	        echo "notarytool log for $$app (id=$$submission_id):"; \
...
notarize-dmg:
	...
	for img in $(DMG) $(DMG_INTEL) $(DMG_ARM); do \
	    if [ -f "$$img" ]; then \
	        ...
	        xcrun stapler validate "$$img"; \
	    fi; \
	done

# sign-minisign produces `<dmg>.minisig` sibling files. The private key is read
# from MINISIGN_KEY (default ~/.minisign/splitwg.key). When MINISIGN_PASSWORD
# is exported, it is piped to minisign's stdin (for CI); otherwise minisign
# prompts for the passphrase interactively.
sign-minisign:
	@set -euo pipefail; \
	for img in $(DMG) $(DMG_INTEL) $(DMG_ARM); do \
	    if [ -f "$$img" ]; then \
	        ...
	        if [ -n "$${MINISIGN_PASSWORD:-}" ]; then \
	            printf '%s\n' "$$MINISIGN_PASSWORD" | minisign -S -s "$(MINISIGN_KEY)" -m "$$img"; \
	        else \
...
# minisign key.
release: notarize-app dmg dmg-intel dmg-arm notarize-dmg sign-minisign
	@echo ""
