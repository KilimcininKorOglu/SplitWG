...
/// Why a connect attempt failed — used by every connect path so each
/// caller can map the variant to its own reporting mechanism.
#[derive(Debug)]
...
/// Shared connect pipeline: sudoers gate → optional Touch ID → `mgr.connect()`.
pub fn ensure_connected(
    ...
    if !auth::is_setup_done() {
        log::info!("splitwg: ensure_connected: sudoers rule missing, running setup");
        auth::run_first_time_setup().map_err(ConnectError::SetupFailed)?;
    }

    if require_auth {
        let prompt = i18n::t_with("auth.touchid_prompt", &[("name", &cfg.name)]);
        match auth::authenticate(&prompt) {
            auth::AuthResult::Success => {}
            auth::AuthResult::NotAvailable => {
                log::info!(
                    "splitwg: ensure_connected: Touch ID not available for {:?}, proceeding",
                    cfg.name
                );
            }
            auth::AuthResult::Denied => return Err(ConnectError::AuthDenied),
        }
    }
...
pub fn spawn_toggle(
    ...
    thread::spawn(move || {
        ...
        let result = match ensure_connected(&mgr, &cfg, false) {
