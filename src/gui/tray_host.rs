...
impl TrayHost {
    ...
    pub fn new(ctx: &egui::Context) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        menu.append(&PredefinedMenuItem::separator())?;
        let about_item = MenuItem::new(i18n::t("tray.menu.about"), true, None);
    ...
    fn refresh(&self, mgr: &wg::Manager) {
        ...
        self.about_item.set_text(i18n::t("tray.menu.about"));
...
fn find_config(name: &str) -> Option<config::Config> {
    log::info!("splitwg: tray: looking up config {:?}", name);
    let cfgs = match config::load_configs() {
        Ok(c) => c,
        Err(e) => {
            log::warn!("gui: tray: load configs: {e}");
            notify::error(&i18n::t("notify.load_failed"), &format!("{name}: {e}"));
            return None;
        }
    };
    let cfg = cfgs.into_iter().find(|c| c.name == name);
    if cfg.is_none() {
...
fn toggle_tunnel(mgr: &wg::Manager, name: &str) {
    ...
    let Some(cfg) = find_config(name) else {
        return;
    };
    ...
    match tasks::ensure_connected(mgr, &cfg, false) {
        Ok(()) => notify::info(&i18n::t("notify.connected"), name),
        Err(tasks::ConnectError::AuthDenied) => {
            log::info!("gui: tray: Touch ID denied for {:?}", name);
        }
        Err(tasks::ConnectError::SetupFailed(msg) | tasks::ConnectError::ConnectFailed(msg)) => {
            notify::error(
