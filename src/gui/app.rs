...
impl App {
    ...
    fn check_watchdog(&mut self, ctx: &egui::Context) {
        ...
        for cfg in &configs {
            ...
            // Handshake info comes from `wg show dump`. If the interface is
            // gone or the read failed, skip — the user's manual state
            // machine will pick it up on the next UI refresh.
            let Some(iface) = wg::interface_for_config(&cfg.file_path) else {
                continue;
            };
            let Some(stats) = wg_stat::query(&iface) else {
                continue;
            };
    ...
    /// Re-reads configs and queries `wg show` stats for active tunnels.
    /// Called once per second while the window is visible.
    fn refresh(&mut self, ctx: &egui::Context) {
        ...
        self.stats.retain(|name, _| {
            self.configs.iter().any(|c| c.name == *name)
                && (self.mgr.is_active(name)
                    || self
                        .configs
                        .iter()
                        .find(|c| c.name == *name)
                        .and_then(|c| wg::interface_for_config(&c.file_path))
                        .is_some())
        });
        ...
        for cfg in &self.configs {
            ...
            if let Some(stats) = wg_stat::query(&iface) {
                // Aggregate peer totals into a single throughput sample.
                ...
                self.stats.insert(cfg.name.clone(), stats);
            }
        }
    ...
    fn maybe_schedule_rtt_probe(&mut self, ctx: &egui::Context) {
        ...
        if wg::interface_for_config(&cfg.file_path).is_none() {
            return;
        }
        if self
            .ping_results
        ...
        tasks::spawn_ping(self.task_tx.clone(), ctx.clone(), name, endpoint);
    }
}
