//! WireGuard tunnel statistics types and display helpers.
//!
//! Stats are now populated by IPC events from the helper (boringtun counters)
//! rather than `wg show dump`.  The `parse()` function and its tests are
//! retained for backwards compatibility with any future `wg` integration.
