...
impl PfAnchor {
    ...
    pub fn load(iface: &str) -> Result<Self> {
        // `pfctl -E` is idempotent: it no-ops when pf is already enabled.
        // We tolerate a non-zero exit here (e.g. pf already on) — the
        // follow-up anchor load is the real signal of success.
        ...
        log::info!("splitwg-helper: pf: anchor {} loaded", anchor);
        Ok(PfAnchor {
            anchor,
    ...
    pub fn unload(&self) {
        ...
        match status {
            Ok(o) if o.status.success() => {
                log::info!("splitwg-helper: pf: anchor {} flushed", self.anchor);
            }
            Ok(o) => {
                log::warn!(
            ...
            Err(e) => log::warn!("splitwg-helper: pf: flush {} error: {e}", self.anchor),
        }
    }
}
