//! MaxMind GeoLite2-Country expansion for `country:XX` rule entries.
//!
//! The user supplies `GeoLite2-Country.mmdb` themselves — we do NOT bundle
//! the database (licensing + redistribute issues, and MaxMind now requires
//! a free account to download). Lookup locations searched, in order:
//!
//! 1. `$HOME/.config/splitwg/GeoLite2-Country.mmdb`
//! 2. `<app bundle>/Contents/Resources/GeoLite2-Country.mmdb`
//!
//! If the mmdb is missing, `expand_country` returns an empty vec and logs
//! a warning; the rule is effectively skipped at bringup time. The Rules
//! tab surfaces a user-visible warning string so the user knows to install
//! the DB.
...
mod tests {
    ...
    fn search_paths_includes_home_config() {
        std::env::set_var("HOME", "/tmp/test-splitwg-home");
        let paths = search_paths();
        assert!(paths.iter().any(|p| p.to_string_lossy().contains(
            "/tmp/test-splitwg-home/.config/splitwg/GeoLite2-Country.mmdb"
        )));
    }

    #[test]
    fn expand_country_without_mmdb_returns_empty() {
        std::env::set_var("HOME", "/tmp/nonexistent-splitwg-home-xxx");
