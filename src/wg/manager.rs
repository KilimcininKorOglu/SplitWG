...
impl Manager {
    ...
    pub fn connect(&self, cfg: &config::Config) -> Result<(), WgError> {
        ...
        let iface = match read_ready(&mut child, READY_TIMEOUT) {
            Ok(iface) => iface,
            Err(e) => {
                let _ = child.kill();
                let _ = child.wait();
    ...
    pub fn reconnect(&self, name: &str) -> Result<(), WgError> {
        // Ignore the "not tracked" error — watchdog may race a manual
        // disconnect, in which case connect() still does the right thing.
...
/// expires. Leaves child.stdout consumed by a background thread that keeps
/// draining subsequent events and discarding them.
fn read_ready(child: &mut Child, timeout: Duration) -> Result<String, String> {
    let stdout = child
        .stdout
        .take()
        ...
        match rx.recv_timeout(remaining.min(Duration::from_millis(500))) {
            Ok(line) => {
                match serde_json::from_str::<IpcEvent>(&line) {
                    Ok(IpcEvent::Ready { iface }) => return Ok(iface),
                    Ok(IpcEvent::Error { message }) => return Err(message),
                    Ok(_) => continue, // ignore Handshake/Stats until Ready
...
// SplitWG 2.0 removes the `wg show` detection path; the free functions below
// preserve their signatures so the UI compiles unchanged while always
// reporting "nothing found". Use `Manager::is_active` for the real answer.

/// Always returns an empty list in v2.0. Kept for API stability.
pub fn active_interfaces() -> Vec<String> {
    Vec::new()
}

/// Always returns `false` in v2.0. Kept for API stability.
pub fn is_connected(_name: &str) -> bool {
    false
}

/// Always returns `None` in v2.0 — external tunnel detection is gone.
pub fn interface_for_config(_cfg_path: &std::path::Path) -> Option<String> {
    None
}
...
mod tests {
    ...
    #[test]
    fn active_interfaces_is_empty() {
        assert!(active_interfaces().is_empty());
    }

    #[test]
    fn is_connected_returns_false() {
        assert!(!is_connected("anything"));
    }

    #[test]
    fn interface_for_config_returns_none() {
        assert!(interface_for_config(std::path::Path::new("/dev/null")).is_none());
    }
