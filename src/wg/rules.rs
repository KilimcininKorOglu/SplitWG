...
pub fn resolve_entries(entries: &[String]) -> Vec<String> {
    log::info!("splitwg: rules: resolving {} entries", entries.len());
    ...
    for raw in entries {
        ...
        if let Some(code) = is_geo_entry(entry) {
            ...
            if expanded.is_empty() {
                log::warn!(
                    "splitwg: rules: country:{} resolved to 0 CIDRs (mmdb missing?)",
                    code
                );
            } else {
                log::info!(
                    "splitwg: rules: country:{} expanded to {} CIDRs",
                    code,
                    expanded.len()
                );
            }
        ...
        let host: String = if is_wildcard_entry(entry) {
            ...
            log::info!(
                "splitwg: rules: wildcard entry {:?} -> resolving base domain {:?}",
                entry,
                base
            );
            base
        ...
        let Ok(iter) = probe.to_socket_addrs() else {
            log::warn!("splitwg: rules: DNS lookup failed for {:?}", host);
            continue;
        };
...
pub fn is_asn_entry(s: &str) -> Option<u32> {
    let num = s.strip_prefix("asn:")?;
    num.parse::<u32>().ok()
}
