...
pub fn decide(rule: &OnDemandRule, state: &NetState) -> Desired {
    ...
    // Schedule gate: evaluated before everything else. When the current
    // local time is outside the window, force Disconnect regardless of the
    // other flags. Missing `local_time` (monitor hasn't run yet) skips the
    // gate so on-demand stays responsive in the first seconds of app boot.
