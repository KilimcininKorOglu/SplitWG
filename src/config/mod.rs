...
pub fn load_configs() -> Result<Vec<Config>, ConfigError> {
    ...
    for e in entries {
        ...
        let rules_path = dir.join(format!("{}.rules.json", name));
...
pub fn ensure_rules_file(name: &str) -> Result<PathBuf, ConfigError> {
    let path = config_dir().join(format!("{}.rules.json", name));
    if !path.exists() {
        let r = Rules::default();
...
pub fn copy_config_file(src: &Path, name: &str) -> io::Result<()> {
    ...
    let dst = config_dir().join(format!("{}.conf", name));
...
pub fn delete_config(name: &str) -> io::Result<()> {
    let dir = config_dir();
    let conf = dir.join(format!("{}.conf", name));
    let rules = dir.join(format!("{}.rules.json", name));
    if conf.exists() {
        fs::remove_file(&conf)?;
    }
    if rules.exists() {
...
pub fn save_rules(name: &str, rules: &Rules) -> Result<(), ConfigError> {
    let path = config_dir().join(format!("{}.rules.json", name));
    log::info!("config: saving rules for {:?} to {}", name, path.display());
    ...
    log::info!("config: rules saved for {:?}", name);
    Ok(())
}
